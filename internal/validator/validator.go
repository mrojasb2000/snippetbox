...
// this pattern once at startup and storing to compiled *regexp.Regexp in a
// variable is more performat than re-parsing the pattern each time we need it.
...
// AddFiedErrors() adds an error message to the FieldErrors map (so long as no
...
// Create an AddNonFieldErrors helper for adding error messages to the new
...
// NoBlank() returns true if a value is not an empty string.
...
// MinChars returns true if a value contains ar least n characters.
